package com.davik.baseapp.model.rxbus;
...
/**
 * Description：Rxbus消息传递
 *      注册：RxBus.getDefault().register(this);
 *
 *      传递：RxBus.getDefault().post(0x1,new Events.FinishDelete());
 *
 *      接收：
             @Subscribe(code = 0x1,threadMode= ThreadMode.MAIN)
             public void enterSelectMode(Events.ShowBottomBar showBottomBar){
                // dosomething
             }
 *
 *      注销：RxBus.getDefault().unRegister(this);
 *
 * Author: Hansion
 * Time: 2016/12/5 12:55
 */
public class RxBus {
